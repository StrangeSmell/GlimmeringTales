...
public class Procreation extends NatureSpellEntry {

    public static final ResourceKey<SpellAction> PROCREATION = spell("procreation");
    public static final ResourceKey<SpellAction> AFFECTING_PROJECTILE = spell("affect_projectile");
    ...
    public void register(BootstrapContext<SpellAction> ctx) {
        new SpellAction(
                procreation(new DataGenContext(ctx), 4, 1.5, 1),
                Items.WHEAT, 100,
...
/*        new SpellAction(
    ...
    private static ConfiguredEngine<?> affectProjectile(DataGenContext ctx, double r, double y, double size) {
        return new ListLogic(List.of(
                new ProcessorEngine(SelectionType.NONE,
                        new SelfSelector(),
                        List.of(
                                new AffectingProjectilesProcessor(
                                        IntVariable.of("4")
                                )
                        )),
                new DelayedIterator(
